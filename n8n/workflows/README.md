...
| `WF-01-order-intake.json` | Webhook WC → HMAC → fetch order → append-then-verify sheet → email + WA link form | T2.11–T2.15 · R1 |
...
| `WF-04-rekap-komisi.json` | ⛔ **DINONAKTIFKAN R1 (2026-08-09)** — tidak ada komisi di model grosir (B1/B2). Berkas disimpan, jangan diimpor ulang | T3.11 · R1 |
...
**Kontrak `/webhook/daftar-reseller`** — ⚠️ **tanpa implementasi sisi situs sejak R1 (2026-08-09):** `page-jadi-reseller.php` + `reseller.js` **dihapus** bersama seluruh program reseller (B1/B2). Webhook sengaja dibiarkan hidup sampai **M7** menulisnya ulang jadi onboarding mitra. Kontraknya sekarang: POST `nama`, `wa`, plus honeypot `website` yang HARUS kosong. **`bank` & `norek` dicabut** — tidak divalidasi, tidak disimpan, tidak disiarkan. Respons JSON `{ok, message}`; 200 = sukses/duplikat, 422 = data kurang / bukan nomor WA, 429 = rate limit.
...
2. Import WF-01, WF-02, WF-03, WF-05, **WF-06**, WF-07, WF-08 — **delapan berkas termasuk WF-00**. ⚠️ **WF-04 sengaja TIDAK ada di daftar ini** — berkasnya dipindah ke `arsip/` di R1 supaya impor-semua tidak menghidupkannya kembali; kodenya masih menyiarkan nomor rekening lewat WhatsApp. Buka tiap node **Google Sheets** / **HTTP dengan Basic Auth** → pilih credential yang dibuat di atas (referensi credential tidak ikut terbawa saat import) → pada node Sheets, klik refresh mapping kolom agar schema terbaca.
...
4. **Activate** semuanya — bila lewat CLI, ingat `import:workflow` **menonaktifkan** workflow: publish kedelapan id lalu `docker restart harih-n8n`, lalu hitung ulang `n8n list:workflow --active=true` (harus **8** sejak R1; sebelumnya 9). Lihat jebakan ketiga di bawah. URL produksi:
   ...
   - WF-03: `https://n8n.harih.id/webhook/daftar-reseller` (⚠️ **tanpa halaman pendaftar sejak R1** — `page-jadi-reseller.php` dihapus, bukan cuma diturunkan. Webhook dibiarkan aktif sampai M7 menulisnya ulang jadi onboarding mitra; sampai saat itu ia hanya bisa membuat baris `PENDING`, dan kupon `RES-` tetap butuh klik approval owner) + `…/webhook/approve-reseller` (hanya diklik owner)
...
4. ~~**Komisi**~~ — ⛔ **dicabut R1.** Tidak ada komisi dihitung di WF-01; tab `komisi` dibekukan (tidak dihapus, tidak diisi lagi). Uji sebaliknya: order dengan kupon apa pun → **tidak** ada baris baru di tab `komisi`.
...
8. **WF-03**: daftar via curl (`nama`, `wa` — R1: `bank`/`norek` sudah tidak diterima; kirim pun akan diabaikan, dan **tidak boleh** muncul di sheet/WA/email) → baris PENDING + owner terima link approval; klik link → kupon muncul di WC, status `AKTIF`, welcome kit + caption masuk WA; klik link **kedua kali** → "sudah aktif", tidak ada kupon ganda; daftar ulang nomor sama → "sudah terdaftar"; isi field `website` → sukses palsu tanpa baris baru; cek kupon di WC: 10%, individual use, limit 1×/user.
9. ~~**WF-04**~~ — ⛔ **dinonaktifkan R1.** Uji sebaliknya: `list:workflow --active=true` harus **8**, dan Senin pagi tidak ada rekap komisi masuk WA owner.
