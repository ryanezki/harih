...
**Kondisi:** infra LIVE dua-duanya. WordPress+tema+form di `https://harih.id` (demo `/u/demo-tema-01/` ✓). Mesin otomasi di VPS `31.97.50.197` (`/opt/harih`): `harih-n8n` + `harih-waha` 2026.6.2. **Baru:** seluruh workflow inti T2 selesai sebagai JSON siap-import di `n8n/workflows/` — WF-00 (error), WF-01 (order intake), WF-02 (generate undangan, 41 node), WF-07 (monitor sesi WAHA); panduan lengkap import/credential/uji di `n8n/workflows/README.md`. Compose menambah env baru (`NODE_FUNCTION_ALLOW_BUILTIN=crypto`, `OWNER_EMAIL`, `OWNER_WA`, `BREVO_SENDER_*`) — **server belum di-redeploy**; placeholder `OWNER_EMAIL`/`OWNER_WA` di `vps/.env` lokal masih harus diisi nilai asli.
...
3. Import 4 workflow + buat credentials UI (`Google SA hariH`, `WC REST hariH`, `WP REST hariH`) + tambah kolom `wa_status`/`exec_id` di tab `orders` + set `N8N_FORM_WEBHOOK_URL` di wp-config + webhook WC T1.19 — semua langkah urut di `n8n/workflows/README.md`.
4. Lanjut pekerjaan inti berikutnya: **produk & toko** (T1.16–T1.18 — nama produk WAJIB memuat kata Hemat/Favorit/Premium; deteksi paket WF-01 bergantung padanya) + katalog (T1.20) → lalu uji end-to-end order sandbox (DoD Minggu 2).
5. Setelahnya: WF-03–06 (reseller, rekap komisi, reminder, backup), template email Brevo (T2.5), musik library (T1.15).
