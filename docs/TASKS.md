...
- [ ] **T2.7 [+]** Exclude `/isi-data/` dari cache LiteSpeed + kirim header no-cache dari template — validasi token adalah PHP per-request; halaman ter-cache berarti validasi tidak berjalan → **kode sudah kirim** `nocache_headers()` + action `litespeed_control_set_nocache`; sisa: setting *Cache → Excludes → URI `/isi-data/`* di LiteSpeed UI (sudah tercantum di output script setup)
