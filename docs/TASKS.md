...
**Update 2026-07-22 (3) — SMOKE TEST 19/19 HIJAU 🟢:** langkah 6 (FluentSMTP→Brevo) & legal SELESAI — 4 halaman legal live (placeholder diisi: kontak hi@harih.id / +62 822-5197-5575, alamat Jalur Sutera Blvd No. 43 Alam Sutera Pinang Tangerang, refund default 7 hari; md di `docs/konten-legal/` = versi tayang). Plugin Duitku terinstall; **sisa terakhir menuju uji bayar: kredensial SANDBOX** — daftar di sandbox.duitku.com (terpisah & instan, TIDAK menunggu formulir aplikasi akun personal — itu untuk production/T0.11) → merchant code + API key sandbox → isi di WooCommerce → Payments → Duitku (mode sandbox) → uji checkout dari HP (T1.18) → setelah approval production keluar: ganti kredensial + uji Rp 10.000 (T4.8/T4.12).

**Update 2026-07-22 (2) — E2E LULUS ✅:** order uji #29 (via WP-CLI, status processing) mengalir tanpa sentuhan: WF-01 → baris sheet + WA/email link form (`wa_status=TERKIRIM`) → form disubmit (2 foto + QRIS + spoof `paket=premium`) → WF-02 → undangan `/u/raka-sela-e998/` terbit di tema-02 (enforcement OK: paket tetap `favorit`, `video_url` dibuang), delivery WA+email+QR, sheet `SUDAH_JADI` + `mempelai`, order `completed`. Idempotency: submit form ke-2 → 200 + link lama; webhook dobel ber-signature → terserap, baris tetap 1. **= DoD Minggu 2 tercapai** (jalur pembayaran Duitku-nya sendiri menyusul di T1.18/T4.8). Tiga bug versi n8n 2.29 ditemukan & difix saat uji: (1) `columns.schema` wajib terisi di node Sheets defineBelow — schema kini dibake di JSON repo; (2) import me-reset flag aktif + aktivasi kini `publish:workflow`; (3) **import by-name menjodohkan credential httpBasicAuth secara serampangan** — dua node WP di WF-02 sempat tertaut ke credential WC (401); ID credential kini dibake di JSON repo (`GOOGLESAHARIH001`/`HARIHWCREST00001`/`HARIHWPREST00001`). Artefak uji masih live utk review owner: order #29 + `/u/raka-sela-e998/` — hapus setelah puas (`wp post delete` + batalkan order). Sisa aksi owner: langkah 6 (FluentSMTP) + 7 (legal + Duitku sandbox → uji checkout riil dari HP).

**Update 2026-07-22 (1):** panduan langkah 1–3 SELESAI — akun n8n diamankan owner; `vps/google-sa.json` ada (SA `n8n-harih@gen-lang-client-0926607889`, dibuat baru — T0.8 lama ternyata tidak pernah jadi); header 3 tab sheet ditulis via Sheets API (orders 15 kolom, resellers 7, komisi 6 — sekaligus membuktikan key+API+share benar). Credential `Google SA hariH` terimpor, ketiga credential tertaut by-ID ke semua node, `errorWorkflow` = WF-00 terpasang di 7 WF via export-patch-reimport, **7 workflow AKTIF** (WF-07 menunggu QR; WF-00 ikut aktif — n8n baru mewajibkan error workflow aktif). Fix penting: `N8N_BLOCK_ENV_ACCESS_IN_NODE=false` di compose — n8n baru memblokir `$env` di Code node secara default. Probe: ping 200 · token salah 403 · reseller kosong 422 ✓. **Sisa aksi owner: langkah 4 (mailbox) · 5 (scan QR) · 6 (FluentSMTP) · 7 (legal+Duitku) → lalu "lanjut" utk E2E.**

**Aksi owner — panduan langkah-demi-langkah lengkap di `docs/panduan-go-live.md`.** Ringkasan urutan:
...
- [ ] **T1.6 [+]** `define('DISABLE_WP_CRON', true)` + cron job nyata di hPanel Hostinger tiap menit (`wp cron event run --due-now`) — delivery webhook WooCommerce lewat Action Scheduler yang bergantung traffic; tanpa cron nyata, order di jam sepi baru terkirim ke n8n saat ada pengunjung berikutnya → `DISABLE_WP_CRON` diset ✓; cron hPanel dipasang owner (2026-07-08) — verifikasi: antrean due-now harus turun ke ~0 (sedang dipantau)
