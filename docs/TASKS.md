...
3. Import WF-00/WF-01 + buat credentials UI (`Google SA hariH`, `WC REST hariH`) + tambah kolom `wa_status`/`exec_id` di tab `orders` + webhook WC T1.19 — semua langkah urut di `n8n/workflows/README.md`.
4. Lanjut pekerjaan inti: **WF-02 generate undangan** (token, `DIPROSES`, enforcement paket, validasi file, upload media, QR, delivery — T2.16–T2.22), tiru fungsi normalisasi nomor dari WF-01.
5. Setelahnya: WF-03–06, produk & katalog (T1.16–T1.21), template email Brevo (T2.5).
