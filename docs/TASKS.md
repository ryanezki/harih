...
- [ ] **A5** 🤝 `jam` — **Penjaga anti-checkout-kosong pada filter gateway + verifikasi ID gateway Duitku**
  `cetak.php:284-288` menghapus setiap gateway yang id-nya tidak memuat `_va_`, `briva`, `indomaret`, `alfamart` untuk keranjang > Rp 2 juta — **tanpa penjaga "kalau hasilnya kosong, kembalikan aslinya"**.
  **ID gateway nyata sudah diambil 2026-08-07** (sambil mengerjakan A4) — dua yang aktif: **`duitku_briva`** dan **`duitku_nobu_qris`**. Sisanya terdaftar tapi `enabled=no` (`duitku_bnc`, `duitku_atome`, `duitku_va_bsi`, `duitku_va_danamon_h2h`, `duitku_jenius_pay`, dst.), plus `bacs`/`cheque`/`cod` yang semuanya mati.
  **Kabar baiknya, skenario terburuk tidak terjadi hari ini:** `duitku_briva` memuat `briva` sehingga lolos saringan — keranjang Rp 2,9 jt & 5,9 jt tetap punya satu tombol bayar. **Tapi `duitku_nobu_qris` disaring habis**, jadi QRIS hilang persis di paket termahal, dan seluruhnya bergantung pada satu gateway: matikan `duitku_briva` sekali saja, checkout paket besar jadi kosong tanpa peringatan.
