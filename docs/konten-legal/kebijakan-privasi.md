# Kebijakan Privasi — hariH

> **DRAF v1 (2026-07-07)** — review & sesuaikan sebelum dipublikasikan sebagai halaman `/kebijakan-privasi/`. Bukan nasihat hukum. Placeholder `{{…}}` wajib diisi.

Terakhir diperbarui: {{TANGGAL_PUBLISH}}

hariH (harih.id) menghormati privasi Anda. Kebijakan ini menjelaskan data apa yang kami kumpulkan, untuk apa, dan hak Anda — disusun dengan memperhatikan UU No. 27 Tahun 2022 tentang Pelindungan Data Pribadi.

## 1. Data yang Kami Kumpulkan

**Dari pemesan:**
- Data pemesanan: nama, alamat email, nomor WhatsApp.
- Data undangan yang Anda isi: nama mempelai & orang tua, tanggal/waktu/lokasi acara, kisah, foto, pilihan musik, nomor rekening & gambar QRIS (bila memakai fitur amplop digital), nomor WhatsApp narahubung.

**Dari tamu undangan:**
- Isian buku ucapan/RSVP: nama, ucapan, dan status kehadiran. Kami tidak meminta kontak tamu.

**Otomatis:** log teknis standar server (alamat IP, waktu akses) untuk keamanan dan pencegahan penyalahgunaan.

## 2. Penggunaan Data

- Membuat, menerbitkan, dan mengirimkan undangan digital Anda.
- Komunikasi transaksional: konfirmasi pembayaran, link pengisian data, pengiriman undangan, pengingat menjelang acara.
- Operasional internal, pencatatan pesanan, dan dukungan pelanggan.

Kami **tidak menjual** data pribadi Anda kepada pihak mana pun.

## 3. Halaman Undangan & Visibilitas

Halaman undangan dapat diakses oleh **siapa pun yang memiliki link-nya** — itulah fungsinya. Kami mencegah halaman undangan diindeks mesin pencari (noindex) dan tidak mencantumkannya di peta situs. Kendali penyebaran link berada pada Anda; pertimbangkan informasi apa yang Anda tampilkan (mis. nomor rekening pada amplop digital hanya muncul sesuai paket dan atas pilihan Anda).

## 4. Pihak Ketiga Pemroses

Data dibagikan terbatas sesuai keperluan layanan kepada:

| Pihak | Keperluan |
|---|---|
| Duitku | pemrosesan pembayaran |
| Brevo | pengiriman email transaksional |
| WhatsApp | pengiriman pesan transaksional |
| Google (Workspace/Sheets) | pencatatan operasional pesanan |
| Hostinger | infrastruktur hosting situs |

Masing-masing tunduk pada kebijakan privasinya sendiri.

## 5. Penyimpanan & Retensi

- Data undangan dan foto disimpan selama masa aktif paket, dan dihapus/diarsipkan setelahnya (paling lambat {{90 hari}} setelah masa aktif berakhir, kecuali diwajibkan lain oleh hukum).
- Data pemesanan dan pembayaran disimpan sesuai kebutuhan pembukuan.
- Cadangan (backup) rutin disimpan terenkripsi/terbatas akses dengan retensi {{4 minggu}}.

## 6. Keamanan

Kami menerapkan langkah wajar: HTTPS di seluruh situs, pembatasan akses data, token pribadi untuk formulir pengisian data, dan pembatasan akses API. Tidak ada sistem yang 100% aman; kami akan memberi tahu Anda bila terjadi insiden yang berdampak signifikan terhadap data Anda.

## 7. Hak Anda

Anda berhak meminta akses, koreksi, atau penghapusan data pribadi Anda (termasuk penghapusan halaman undangan sebelum masa aktif berakhir, dan penghapusan ucapan tamu tertentu). Ajukan melalui halaman [Kontak](/kontak/); kami tanggapi selambatnya {{7 hari kerja}}.

## 8. Cookie

Situs memakai cookie fungsional seperlunya (keranjang belanja & proses checkout). Kami tidak memakai cookie iklan pihak ketiga. {{Sesuaikan bila memasang analytics.}}

## 9. Perubahan Kebijakan

Perubahan diumumkan di halaman ini beserta tanggal pembaruan.

## 10. Kontak

Pertanyaan privasi: email {{cs@harih.id}} · WhatsApp {{+62 8xx-xxxx-xxxx}}.
